=head1 OpenAFS Newsletter, Volume 1, Issue 1, May 2009
...
various development efforts and new of OpenAFS for the community. Please
...
Jake is working on this in his spare time.

=head3 git migration

The effort to do a bulk git conversion of the OpenAFS CVS history
fizzled. no fully automated way was found to do the conversion because CVS
was used in non-standard ways. None of the available conversion tools
could do the job. Even writing custom tools failed.

Simon Wilkinson says we'll probably do a naive migration and rebase the
history at some point. Simon would like to have this done before the
Google Summer of code gets into full swing. There is a desire to use
gerrit to allow for better code review.

Ref: http://jabber.openafs.org/openafs@conference.openafs.org/2009-05-04.txt


=head3 Merging the 1.4 and 1.5 branches

Jeff, Derrick, and others want to merge the changes from 1.4 into the 1.5
branch. The plan is to make the 1.5 the new stable branch. The 1.5 branch
is usable on Linux and other Unix platforms, but it needs more testing
before it can become the stable branch. Testers are wanted, especially for
the demand-attached fileserver compile option. Bug reports welcome.
